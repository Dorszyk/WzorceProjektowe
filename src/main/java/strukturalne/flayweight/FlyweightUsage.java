...
public class FlyweightUsage {
    ...
    public static void main(String[] args) {
      ...
      /* LUB w najnowszej wersji Javy
      switch (carType) {
        case 0 -> producedCars.add(carFactory.createSkodaCityGo(UUID.randomUUID().toString(), getVersion()));
        case 1 -> producedCars.add(carFactory.createVWGolf(UUID.randomUUID().toString(), getVersion()));
        case 2 -> producedCars.add(carFactory.createVWPolo(UUID.randomUUID().toString(), getVersion()));
        case 3 -> producedCars.add(carFactory.createVWPoloGTI(UUID.randomUUID().toString(), getVersion()));
      }
       */


        System.out.println("I created " + producedCars.size() + " cars, but only " + Engine.instances + " references to Engine object");
        // zostanie wypisane: 'I created 1000 cars, but only 4 references to Engine object'
    }
